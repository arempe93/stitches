...
const transformPropertyData = (name, data, themeMap) =>
	...
	: String(data).replace(
		captureTokens,
		($0, direction = '', multiplier = '1', token) => (
			direction || multiplier !== '1' ? 'calc(' : ''
		) + 'var(-' + (
			!/\$/.test(token) && name in themeMap ? '-' + themeMap[name] : ''
		) + '-' + token.replace(/\$/g, '-') + ')' + (
			direction || multiplier !== '1' ? '*' + (direction === '+' ? '' : direction) + multiplier + ')' : ''
		),
	)
			...
			for (let name of Reflect.ownKeys(style)) {
				...
				loop: for (let data of dataList) {
					...
					if (isDeclaration(data)) {
						...
						if (name in utils && !(name == visitedName && style == visitedStyle)) {
